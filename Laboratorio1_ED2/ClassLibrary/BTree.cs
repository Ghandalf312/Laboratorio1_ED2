...
namespace ClassLibrary
{
    class BTree<T> where T : IComparable, IFixedSizeText, new()
    {
        ...
        private void Remove(BNode<T> node, T val)
        {
            bool deleted = false;
        ...
        private void Rearrange(BNode<T> node)
        {
        ...
        private void LendFromBrother(BNode<T> node, BNode<T> brother, BNode<T> father, bool fromRight)
        {
            if (fromRight)
            {
                for (int i = 0; i < Degree - 1; i++)
        ...
        private void JoinNodes(BNode<T> left, BNode<T> right, BNode<T> father)
        {
            for (int i = 0; i < Degree - 1; i++)
            {
